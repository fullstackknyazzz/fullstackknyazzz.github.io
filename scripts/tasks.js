import { totalTasksElement, completedTasksElement, todoList, todoEmptyList } from './main.js';
import { todoSummaryCompleteAll, completedTasks, removeTaskFromLocalStorage, allTasks} from './storage.js';
...
export function deleteTask(event) {
    ...
    parentNode.remove();
    const deleteElement = parentNode.querySelector('.todo_task_page ');
    const taskId = Number(parentNode.closest('li').id);
    removeTaskFromLocalStorage(taskId);


    allTasks.count = updateTaskSummary(allTasks.count, 'delete');
    ...
    if (deleteElement.classList.contains('completed')) {
...
export function updateTaskSummary(count, type) {
    if (type === 'add') {
        count += 1;
    } else if (type === 'delete') {
        count -= 1;
    }
    totalTasksElement.innerText = count;
    todoSummaryCompleteAll.innerText = count;
    return count;
}

export function completedToggleTask(event) {

    const parentNode = event.target.parentNode.parentNode.querySelector('.todo_task_page ');
